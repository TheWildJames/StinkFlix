...
import {
  ...
  Download, Search, SortAsc, Heart, Calendar, Plus, List, FolderPlus
} from 'lucide-react';
...
type ViewTab = 'my-list' | 'custom-lists';
...
export default function Watchlist() {
  ...
  const [activeView, setActiveView] = useState<ViewTab>('my-list');
  const [showCreateList, setShowCreateList] = useState(false);
  const [customLists, setCustomLists] = useLocalStorage<CustomList[]>('stinkflix-custom-lists', []);
    ...
    <div className="min-h-screen bg-[#0a0a0f] pt-20 page-transition">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex gap-2 mb-6">
          ...
          <button
            onClick={() => setActiveView('custom-lists')}
            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium transition-all ${
              activeView === 'custom-lists'
                ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30'
                : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white border border-white/10'
            }`}
          >
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl font-medium transition-all text-sm shadow-lg shadow-emerald-500/30"
              >
                <Plus size={16} />
                Create List
              </button>
            ...
            {customLists.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {customLists.map(list => (
                  <div
                    key={list.id}
                    className="group relative bg-white/5 border border-white/5 hover:border-emerald-500/30 rounded-2xl overflow-hidden transition-all hover:shadow-xl hover:shadow-emerald-500/10 hover:-translate-y-1"
                  >
                    {/* Color header */}
                    <div className={`h-24 bg-gradient-to-br ${list.color} relative`}>
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 p-1.5 bg-black/30 hover:bg-red-500/80 text-white/70 hover:text-white rounded-lg transition-all opacity-0 group-hover:opacity-100"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
              ...
              <div className="text-center py-24">
                <FolderPlus size={56} className="mx-auto text-white/10 mb-5" />
                <p className="text-white/30 text-lg font-medium">No custom lists yet</p>
                <p className="text-white/20 text-sm mt-2">Create your first custom list to organize your favorites</p>
              </div>
            )}
          </div>
        )}
      ...
      {/* Create List Modal */}
      {showCreateList && (
        <div className="fixed inset-0 z-[9998] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowCreateList(false)}>
          <div className="bg-[#1a1a2e] rounded-2xl max-w-md w-full border border-white/10 animate-slide-up" onClick={e => e.stopPropagation()}>
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-white">Create New List</h3>
                <button onClick={() => setShowCreateList(false)} className="text-white/40 hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
              <div>
